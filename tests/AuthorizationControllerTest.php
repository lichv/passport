...
class AuthorizationControllerTest extends TestCase
{
    ...
    {
        Lichv\Passport\Passport::tokensCan([
            'scope-1' => 'description',
        ]);
        ...
        $controller = new Lichv\Passport\Http\Controllers\AuthorizationController($server, $response);
        ...
        $authRequest->shouldReceive('getScopes')->andReturn([new Lichv\Passport\Bridge\Scope('scope-1')]);
        ...
        $clients = Mockery::mock('Lichv\Passport\ClientRepository');
        $clients->shouldReceive('find')->with(1)->andReturn('client');

        $tokens = Mockery::mock('Lichv\Passport\TokenRepository');
    ...
    public function test_request_is_approved_if_valid_token_exists()
    {
        ...
        $authRequest->shouldReceive('getScopes')->once()->andReturn([new Lichv\Passport\Bridge\Scope('scope-1')]);
        ...
        $tokens = Mockery::mock('Lichv\Passport\TokenRepository');
        $tokens->shouldReceive('findValidToken')->with($user, 'client')->andReturn($token = Mockery::mock('Lichv\Passport\Token'));
